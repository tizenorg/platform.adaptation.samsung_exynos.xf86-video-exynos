                                  ...
                                  pointer data)
{
    ...
    {
        *value = pPort->rotate;
        XDBG_TRACE(MIA, "Get Attribute ROTATION Value %li\n", *value);
        return Success;
    }
    else if (attribute == _exynosVideoImageGetPortAtom (PAA_HFLIP))
    {
        *value = pPort->hflip;
        XDBG_TRACE(MIA, "Get Attribute HFLIP Value %li\n", *value);
        return Success;
    }
    else if (attribute == _exynosVideoImageGetPortAtom (PAA_VFLIP))
    {
        *value = pPort->vflip;
        XDBG_TRACE(MIA, "Get Attribute VFLIP Value %li\n", *value);
        return Success;
    }
                                  ...
                                  pointer data)
{
    ...
    {
        pPort->rotate = value;
        XDBG_DEBUG(MIA, "port(%d) rotate(%li) \n", pPort->index, value);
        return Success;
    }
    else if (attribute == _exynosVideoImageGetPortAtom (PAA_HFLIP))
    {
        pPort->hflip = value;
        XDBG_DEBUG(MIA, "port(%d) hflip(%li) \n", pPort->index, value);
        return Success;
    }
    else if (attribute == _exynosVideoImageGetPortAtom (PAA_VFLIP))
    {
        pPort->vflip = value;
        XDBG_DEBUG(MIA, "port(%d) vflip(%li) \n", pPort->index, value);
        return Success;
    }
    else if (attribute == _exynosVideoImageGetPortAtom (PAA_OUTPUT))
    {
...
//    int output;
    ...
    /** @todo Temprorary part for testing */
//    pPort->drawing = ON_FB;
//    pPort->d.dst.x = pPort->d.pDraw->x;
//    pPort->d.dst.y = pPort->d.pDraw->y;
...
//    if (!pPort->disable_clone)
//    {
//        exynosTvoutDisableClone(pScrn);
//        pPort->disable_clone = TRUE;
//    }
...
//    }
    ...
    if (pPort->prop_id)
    {
        if(_exynosVideoImageCheckChange (pPort))
        {
            pExynos->pEngine->FinishedConvert(pPort->prop_id);
            pPort->prop_id = 0;
        }
    }
...
//    }
    ...
    outbuf =  _exynosVideoImageGetDrawPixmap(pPort->d.pDraw);
    if (!pPort->prop_id)
        pPort->prop_id = (pExynos->pEngine->PrepareConvert)(pPort->in_width, pPort->in_height,pPort->d.fourcc, &(pPort->in_crop),
                                     pPort->out_width, pPort->out_height, FOURCC_RGB32, &(pPort->out_crop),
                                     pPort->hflip, pPort->vflip, pPort->rotate);
    if (!pPort->prop_id)
        return BadRequest;
    EXYNOSExaPrepareAccess(outbuf, 0);
    if (!(pExynos->pEngine->SetBuffer)(pPort->prop_id, (void*)buf, (void*) outbuf->devPrivate.ptr))
    {
        return BadRequest;
    }
    if (!(pExynos->pEngine->SyncConverter)(pPort->prop_id))
    {
        return BadRequest;
    }

    EXYNOSExaFinishAccess(outbuf, 0);
    DamageDamageRegion (pPort->d.pDraw, pPort->d.clip_boxes);
...
//    if (pPort->disable_clone)
//    {
//        exynosTvoutEnableClone(pScrn);
//        pPort->disable_clone = FALSE;
//    }
...
//
//    XDBG_TRACE (MIA, "pDraw(0x%08x) video-only(%s)\n",
