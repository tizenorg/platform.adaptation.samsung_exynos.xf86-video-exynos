...
g2d_exec (void)
{
    ...
    exec.async = 0;
    ret = drmIoctl(gCtx->drm_fd, DRM_IOCTL_EXYNOS_G2D_EXEC, &exec);
    if (ret < 0) {
...
g2d_flush (void)
{
    ...
    cmdlist.cmd_buf = (unsigned long)&gCtx->cmd_gem[0];
    cmdlist.cmd_nr = gCtx->cmd_nr;
    cmdlist.cmd_buf_nr = gCtx->cmd_gem_nr;
    cmdlist.event_type = G2D_EVENT_NOT;
    cmdlist.user_data = 0;

    gCtx->cmd_nr = 0;
    gCtx->cmd_gem_nr = 0;
    ret = drmIoctl(gCtx->drm_fd, DRM_IOCTL_EXYNOS_G2D_SET_CMDLIST, &cmdlist);
    if (ret < 0) {
...
g2d_image_new(void)
{
    ...
    img = ctrl_calloc(1, sizeof(G2dImage));
    if(img == NULL)
    {
                                            ...
                                            void* data, unsigned int stride)
{
    ...
    if(data)
    {
        struct drm_exynos_g2d_userptr userptr;

        memset(&userptr, 0, sizeof(struct drm_exynos_g2d_userptr));
