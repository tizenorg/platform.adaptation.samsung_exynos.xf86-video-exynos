...
{
    tbo_item* need_bo=_find_bo(bo);
    return need_bo->user_data=data_free_func;
}
...
void print_tbm(tbo_item*bo){
    printf(" obj (%x) ref_cnt(%i) map_cnt(%i) size(%i)",bo->ptr,bo->ref_cnt,bo->map_cnt,bo->size);
}
...
tbm_bo_forTests( tbm_bo bo, unsigned int val )
{
    tbo_item* need_bo=_find_bo(bo);
    need_bo->user_data= val;
}
