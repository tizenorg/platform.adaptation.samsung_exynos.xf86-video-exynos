...
static int node_fixup(struct udev_device *dev, mode_t mode, uid_t uid, gid_t gid, const char *smack)
{
        struct udev *udev = udev_device_get_udev(dev);
        ...
        if (smack != NULL) {
                if (smack_lsetlabel(devnode, smack, SMACK_LABEL_ACCESS) != 0) {
                        err = -errno;
                        info(udev, "setting smack label '%s' for node '%s' failed\n", smack, devnode);
                }
        }
...
void udev_node_add(struct udev_device *dev, mode_t mode, uid_t uid, gid_t gid, const char *smack)
{
        struct udev *udev = udev_device_get_udev(dev);
        ...
        if (node_fixup(dev, mode, uid, gid, smack) < 0)
                return;
